...
#     import combining_data
    ...
    parser.add_argument('--WEEK_OFFSET', type=int, default=8, help='param for hiding last X weeks (used only when preparing training data, set to 0 for prediction), default 0')
...
#     surv_df.to_csv(f"{base_dir}/test/test.csv", header=False, index=False)
    ...
    weekly_df = utils.preprocess_weekly_data(dynamic_df,static_df, args.MIN_TRANSACTION_WEEKS, args.WEEK_OFFSET, random_cutoff_week_for_active=True)
...
#     last_trx_cutoff_week_df = pd.read_sql_query(sql=last_trx_cutoff_week_query.format(**DATA_MAP), con=engine)
    ...
    full_df.to_csv(f"{base_dir}/train/train.csv", header=True, index=False)
    ...
    today = date.today()
    archive_file_name = 'train-' + str(today) + '.csv'
    full_df.to_csv(f"{base_dir}/train_archive/{archive_file_name}", header=True, index=False)