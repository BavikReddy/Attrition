...
survey AS (
    ...
    from prod_share.edl_repl.repl_sf_gold_post_call_survey surv
    inner join (select distinct s.accountid,s.account_code_from_account__c from prod_share.edl_repl.repl_sf_gold_contact s) sf_acc ON surv.account__c = sf_acc.accountid 
    ...
    left join prod_share.edl_repl.REPL_MART_CALENDAR CAL ON CAL.WEEK_STARTS = DATE_TRUNC('WEEK',NVL(surv.survey_response_date__c,surv.createddate))
