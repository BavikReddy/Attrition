...
call_cases AS (
  ...
  from edl.edl_repl.repl_sf_gold_case cas
  inner join (select distinct s.accountid,s.account_code_from_account__c from edl.edl_repl.repl_sf_gold_contact s) sf_acc ON sf_acc.accountid = cas.accountid
  ...
  left join edl.edl_repl.REPL_MART_CALENDAR CAL ON CAL.WEEK_STARTS = DATE_TRUNC('WEEK',cas.createddate)
