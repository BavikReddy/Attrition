...
from edl.edl_repl.REPL_MART_CALENDAR CAL WHERE CAL.WEEK_NO='{current_week}'
),
...
(
    ...
    FROM  edl.edl_repl.repl_nat_account_attributes NAT
...
(
    SELECT 
     ...
     FROM
                edl.edl_repl.repl_geac_501_record
...
BILL_FREQ AS(
    ...
    FROM(
           ...
           FROM
                 edl.edl_repl.repl_ccex_customer CU
          LEFT JOIN edl.edl_repl.repl_ccex_v_udf_data DA ON DA.CUST_SYSTEM_CODE=CU.CUST_SYSTEM_CODE 
...
(
   SELECT 
              ...
              ST AS CUST_STATE 
   FROM  edl.edl_repl.repl_fin1_tbl_dailyaccountsdownload
),
CREDIT_SCORE AS
(
  ...
  FROM  edl.edl_repl.repl_fin1_tbl_smallfleetaccounts APP
),
...
(
     ...
     FROM(
                   ...
                   FROM edl.edl_repl.repl_nat_tbl_summarized_revenue tbl
...
PRODUCT_INFORMATION_SPEND AS (
   ...
   FROM(
      ...
      FROM edl.edl_repl.repl_nat_tbl_summarized_revenue NAT 
...
SELECT ACCTCODE,CHANNEL
    FROM(
      SELECT
           ...
           FROM edl.user_global_sales.nat_bookings_master
...
SELECT ACCTCODE,CHANNEL
    FROM(
         SELECT
           ...
           FROM edl.edl_repl.repl_mart_sfdc_nat_all_deals1
    ...
    FROM(
             SELECT substring(RK508_ACCOUNT_NO  from 10 for 5) AS ACCTCODE,
                      ...
                      ROW_NUMBER() OVER ( PARTITION BY substring(RK508_ACCOUNT_NO  from 10 for 5) ORDER BY [RK508_REM_DT] DESC) RNUM
                       FROM edl.edl_mdl.vw_fin1_geac_rk508b_closed_items
