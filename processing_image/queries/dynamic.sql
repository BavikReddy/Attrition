...
WEEKS_CUST_MAP AS
 (
   SELECT
	...
	ACCTCODE
	FROM edl.edl_REPL.REPL_MART_CALENDAR CAL
...
(
  ...
  FROM                   
	edl.edl_repl.repl_fmlog F     
  LEFT  JOIN  edl.edl_repl.repl_nat_newlob CO 
                       ON FMDL_CUST_ACCOUNT_CODE = CO.ACCTCODE             
  LEFT JOIN edl.edl_repl.REPL_MART_CALENDAR CAL ON CAL.WEEK_STARTS = DATE_TRUNC('WEEK',F.FMDL_rk_transaction_date)
  WHERE 
                 CAL.WEEK_NO BETWEEN to_char(to_date('{current_week}', 'yyyyww') - interval '{history_weeks} weeks', 'yyyyww')::integer AND '{current_week}'::integer
                  AND  CO.FINALLOB = 'NAT'
                  AND FMDL_SC_SRVC_CNTR_CODE NOT IN ('MC901','MC902','MC903','MC904','MC905')
  ...
  FROM(
	       ...
	       CAL.WEEK_NO as YEARWEEK,
	            ...
	            WHEN lpad(STR.SJT_BUCKET_15_GL_NO::VARCHAR,5,'0') IN ('00054') THEN 'LATE FEE INTEREST'
                        END  AS FEE_TYPE 
               FROM   
	       edl.edl_repl.repl_sj_slsdetl_trans_rec  STR 
                         INNER JOIN ACTIVE_CUSTS_UNIQUE  CUST ON CUST.ACCTCODE=(RIGHT(SJT_AR_ACCT_NBR, 5))
                         LEFT JOIN edl.edl_repl.REPL_MART_CALENDAR CAL ON CAL.WEEK_STARTS = DATE_TRUNC('WEEK',STR.SJT_LOAD_DATE  )
              WHERE 
                            CAL.WEEK_NO BETWEEN to_char(to_date('{current_week}', 'yyyyww') - interval '{history_weeks} weeks', 'yyyyww')::integer AND '{current_week}'::integer  
...
--RISK BASED PRICING FEE
 risk_based_pricing_fee AS(
  ...
  FROM
            edl.edl_repl.repl_sj_slsdetl_trans_rec STR
           INNER JOIN ACTIVE_CUSTS_UNIQUE   CUST ON CUST.ACCTCODE=(RIGHT(SJT_AR_ACCT_NBR, 5))
            LEFT JOIN edl.edl_repl.REPL_MART_CALENDAR CAL ON CAL.WEEK_STARTS = DATE_TRUNC('WEEK',STR.SJT_TRAN_DATE)
...
express_code_fee AS (
   ...
   FROM  
            edl.edl_repl.repl_all_settlement_alllocations
         
           INNER JOIN ACTIVE_CUSTS_UNIQUE  CUST ON CUST.ACCTCODE=trim(CUSTOMERACCOUNTCODE)
            LEFT JOIN edl.edl_repl.REPL_MART_CALENDAR CAL ON CAL.WEEK_STARTS = DATE_TRUNC('WEEK',InvoiceDate)
   WHERE
             CAL.WEEK_NO BETWEEN to_char(to_date('{current_week}', 'yyyyww') - interval '{history_weeks} weeks', 'yyyyww')::integer AND '{current_week}'::integer  
...
inactivity_fee AS(
   ...
   FROM
	 edl.edl_repl.repl_sj_slsdetl_trans_rec 
                  INNER JOIN ACTIVE_CUSTS_UNIQUE  CUST ON CUST.ACCTCODE=RIGHT(SJT_AR_ACCT_NBR,5)
                 LEFT JOIN edl.edl_repl.REPL_MART_CALENDAR CAL ON CAL.WEEK_STARTS = DATE_TRUNC('WEEK',SJT_TRAN_DATE)
   WHERE
	(
...
eld_equipment_fee AS (
    ...
    FROM  (
               ...
               FROM   edl.edl_repl.repl_sj_slsdetl_trans_rec STR    
               INNER JOIN edl.edl_repl.repl_sj_products SP
