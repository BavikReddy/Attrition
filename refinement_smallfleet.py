...
def feature_selection(x_train_2021,y_train_2021):
    ...
    # filename_featureselection="feature_selection_smallfleet"+'_'+filename+".csv"
    filename_featureselection="feature_selection_smallfleet.csv"
    featureselection_path=os.path.join(base_dir,"feature_selection_smallfleet",filename_featureselection)
...
#             x_tst = pd.DataFrame(data=y, columns=y.columns)
    ...
    # filename_modelselection="model_selection_smallfleet"+'_'+filename+".csv"
    filename_modelselection="model_selection_smallfleet.csv"
    modelselection_path=os.path.join(base_dir,"model_selection_smallfleet",filename_modelselection)
...
#     best_params_df.to_excel(hyperparameter_tuning_path)
    ...
    feature_dict_path=os.path.join(base_dir,"feature_dict_smallfleet","feature_dict_smallfleet.json")
    ...
    # make a copy
    today = date.today()
    archive_file_name = 'feature_dict_smallfleet-' + str(today) + '.json'
    feature_dict_path_archive = os.path.join(base_dir,"feature_dict_smallfleet_archive", archive_file_name)
...
#         x_test_new.iloc[:,:] = scaler.fit_transform(x_test_new.loc[:,:])
    ...
    # filename_hyperparameter_tuning="hyperparameter_tuning_smallfleet"+'_'+filename+".csv"
    filename_hyperparameter_tuning="hyperparameter_tuning_smallfleet.csv"
    hyperparameter_tuning_path=os.path.join(base_dir,"hyperparameter_tuning_smallfleet",filename_hyperparameter_tuning)
    print("hyperparameter_tuning_path:",hyperparameter_tuning_path)
    best_df.to_csv(hyperparameter_tuning_path, header=True, index=False)
    
    # make a copy
    today = date.today()
    archive_file_name = 'hyperparameter_tuning_smallfleet-' + str(today) + '.csv'
    hyperparameter_tuning_path_archive = os.path.join(base_dir,"hyperparameter_tuning_smallfleet_archive", archive_file_name)
...
if __name__=='__main__':
    ...
    # full_df_small=full_df[full_df['customer_size'].isin(['small'])]

#     #Find max week in last_transaction_week
#     max_week_train=full_df_small['last_transaction_week'].max()
...
#     full_df['churn_flag']=np.where(full_df['last_transaction_week']>int(run_week_train),0,1)
    ...
    full_df_small.drop(['parentname','last_transaction_week',
    ...
    print("shape of train data:",full_df_small.shape)

    # split to x and y
    y_train=full_df_small['churn_flag']
    full_df_small.drop(['churn_flag','credit_limit'],axis=1,inplace=True)
    x_train=full_df_small.copy()
    ...
    #Create small Fleet Model
    print('Predicting Attrition Probability of small fleet customers')
    #Getting only the Small fleet customers
    full_df_small_test=full_df_test[full_df_test['customer_size'].isin(['small', 'medium'])]
    # full_df_small_test=full_df_test[full_df_test['customer_size'].isin(['small'])]
    full_df_small_test=full_df_small_test.reset_index()
    full_df_small_test=full_df_small_test.copy()

    # #Find max week in last_transaction_week
    # max_week_test=full_df_small_test['last_transaction_week'].max()
    ...
    #Remove unwanted columns
    full_df_small_test.drop(['parentname','last_transaction_week',
     'last_transaction_month','customer_size','customer_segment','first_trx_month'],axis=1,inplace=True)
    ...
    full_df_small_test=pd.concat([full_df_small_test.drop(CATEGORICAL_COLS, axis=1), pd.get_dummies(full_df_small_test[CATEGORICAL_COLS], prefix=CATEGORICAL_COLS)], axis=1)

    print("full_df_small_test shape:",full_df_small_test.shape)
    y_test=full_df_small_test['churn_flag']
    x_test=full_df_small_test.copy()
