...
#     x_train_2021.iloc[:,:] = power.fit_transform(x_train_2021.loc[:,:])
...
# #     x_test_new.iloc[:,:] = scaler.fit_transform(x_test_new.loc[:,:])
    ...
    #recursive feature elimination-Lightgbm
    ...
    # filename_featureselection="feature_selection_largefleet"+'_'+filename+".csv"
    filename_featureselection="feature_selection_largefleet.csv"
    featureselection_path=os.path.join(base_dir,"feature_selection_largefleet",filename_featureselection)
...
def model_selection(feature_selection_df,x_train,y_train,x_test,y_test):
    ...
    total_4=all_features[all_features['Total']>=4]['Feature'].to_list()
    
    featuresets = [
                   (x_train[total_6], x_test[total_6], 'total_6'),
                   (x_train[total_5], x_test[total_5], 'total_5'),
                   (x_train[total_4], x_test[total_4], 'total_4'),
...
#             x_tr.iloc[:,:] = power.fit_transform(x_tr.loc[:,:])
#             x_tst.iloc[:,:] = power.fit_transform(x_tst.loc[:,:])


#             #Scaling
#             scaler = StandardScaler()
#             x_tr.iloc[:,:] = scaler.fit_transform(x_tr.loc[:,:])
#             x_tst.iloc[:,:] = scaler.fit_transform(x_tst.loc[:,:])
    ...
    # filename_modelselection="model_selection_largefleet"+'_'+filename+".csv"
    filename_modelselection="model_selection_largefleet.csv"
    modelselection_path=os.path.join(base_dir,"model_selection_largefleet",filename_modelselection)
...
def hyperparameter_tuning(model_comparison,feature_selection_df,x_train,y_train,x_test,y_test):
    ...
    feature_dict['total_4']=all_features[all_features['Total']>=4]['Feature'].to_list()
...
#     hyperparameter_tuning_path=os.path.join('data/largefleet',filename_hyperparameter_tuning)
#     print("hyperparameter_tuning_path:",hyperparameter_tuning_path)
#     best_params_df.to_excel(hyperparameter_tuning_path)
    
    feature_dict_path=os.path.join(base_dir,"feature_dict_largefleet","feature_dict_largefleet.json")
    ...
    # make a copy
    today = date.today()
    archive_file_name = 'feature_dict_largefleet-' + str(today) + '.json'
    feature_dict_path_archive = os.path.join(base_dir,"feature_dict_largefleet_archive", archive_file_name)
...
#         x_test_new.iloc[:,:] = scaler.fit_transform(x_test_new.loc[:,:])
    ...
    # filename_hyperparameter_tuning="hyperparameter_tuning_largefleet"+'_'+filename+".csv"
    filename_hyperparameter_tuning="hyperparameter_tuning_largefleet.csv"
    hyperparameter_tuning_path=os.path.join(base_dir,"hyperparameter_tuning_largefleet",filename_hyperparameter_tuning)
    print("hyperparameter_tuning_path:",hyperparameter_tuning_path)
    best_df.to_csv(hyperparameter_tuning_path, header=True, index=False)
    
    # make a copy
    today = date.today()
    archive_file_name = 'hyperparameter_tuning_largefleet-' + str(today) + '.csv'
    hyperparameter_tuning_path_archive = os.path.join(base_dir,"hyperparameter_tuning_largefleet_archive", archive_file_name)
...
if __name__=='__main__':
    ...
    #Select only large-fleet customers
...
#     max_week_train=full_df_large['last_transaction_week'].max()
...
#     full_df['churn_flag']=np.where(full_df['last_transaction_week']>int(run_week_train),0,1)
    ...
    full_df_large.drop(['churn_flag','credit_limit'],axis=1,inplace=True)
    x_train=full_df_large.copy()
    ...
    #Create large Fleet Model
    print('Predicting Attrition Probability of Large fleet customers')
    #Getting only the Large fleet customers
    # full_df_large_test=full_df_test[full_df_test['customer_size'].isin(['medium','large'])]
    full_df_large_test=full_df_test[full_df_test['customer_size'].isin(['large'])]
    full_df_large_test=full_df_large_test.reset_index()
    full_df_large_test=full_df_large_test.copy()
    
    # #Find max week in last_transaction_week
    # max_week_test=full_df_large_test['last_transaction_week'].max()
    ...
    #Remove unwanted columns
    full_df_large_test.drop(['parentname','last_transaction_week',
     'last_transaction_month','customer_size','customer_segment','first_trx_month'],axis=1,inplace=True)
    ...
    full_df_large_test=pd.concat([full_df_large_test.drop(CATEGORICAL_COLS, axis=1), pd.get_dummies(full_df_large_test[CATEGORICAL_COLS], prefix=CATEGORICAL_COLS)], axis=1)

    print("full_df_large_test shape:",full_df_large_test.shape)
    y_test=full_df_large_test['churn_flag']
    x_test=full_df_large_test.copy()
