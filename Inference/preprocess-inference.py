...
#     last_trx_cutoff_week_df = pd.read_sql_query(sql=last_trx_cutoff_week_query.format(**DATA_MAP), con=engine)
    ...
    full_df.to_csv(f"{base_dir}/inference/inference.csv", header=True, index=False)
    ...
    today = date.today()
    archive_file_name = 'inference-' + str(today) + '.csv'
    full_df.to_csv(f"{base_dir}/inference_archive/{archive_file_name}", header=True, index=False)